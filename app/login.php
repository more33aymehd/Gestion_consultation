...
if (isset($_POST['submit'])) {    
    ...
    $query = "SELECT * FROM admins WHERE nom = '$email' AND mot_de_passe = '$password'";
    ...
    $query = "SELECT * FROM medecins WHERE nom = '$email' AND mot_de_passe = '$password'";
    $res = mysqli_query($conn, $query);
    if (mysqli_num_rows($res) > 0) {
        $med = mysqli_fetch_assoc($res);
        $_SESSION['medecin'] = $med['nom'];
        $_SESSION['id'] = $med['id_medecin'];
        header('location:./Gestion_consultation-Mohammadou\docteur.php');
        exit();
    }

    // Vérifier dans patients
    $query = "SELECT * FROM patients WHERE nom = '$email' AND mot_de_passe = '$password'";
    $res = mysqli_query($conn, $query);
    if (mysqli_num_rows($res) > 0) {
...
<head>    
    ...
    <link rel="stylesheet" href="../Gestion_consultation-Mohammadou/style_1.css">    
</head>    
<body>    
    <div class="form">  
